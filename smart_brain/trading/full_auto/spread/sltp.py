# trading/full_auto/spread/sltp.py
"""
价差止损止盈工人 - 被动接收数据，条件触发执行

工作流程：
1. 被动接收大脑推送的数据
2. 收到标签 {"info": "开启全自动"} → 缓存
3. 收到标签 {"info": "当前策略:价差套利"} → 缓存
...
class SpreadSlTp:
    ...
    def on_data(self, data: Dict[str, Any]):
        ...
        info = data["info"]
        logger.info(f"📥【价差止损止盈工人】收到标签: {info}")
    ...
    def _check_and_execute(self):
        ...
        if (self.auto_mode_active and 
            self.spread_strategy_active and 
            self.okx_open_ok and 
            self.binance_open_ok):
            logger.info("🎯【价差止损止盈工人】四个标签已齐，开始执行")
            self._is_executing = True
            self._current_task = asyncio.create_task(self._execute())
    
    def _deactivate(self):
        """收到结束全自动标签，立刻重置所有状态"""
        logger.info("🛑【价差止损止盈工人】收到结束全自动标签，立刻重置")
        
        if self._current_task and not self._current_task.done():
            self._current_task.cancel()
            logger.info("🛑【价差止损止盈工人】已取消正在执行的任务")
    ...
    async def _execute(self):
        ...
        try:
            ...
            logger.info("✅【价差止损止盈工人】完成")
            
        except asyncio.CancelledError:
            logger.info("🛑【价差止损止盈工人】任务被取消")
        except Exception as e:
            logger.error(f"❌【价差止损止盈工人】执行异常: {e}")
            import traceback
            ...
            try:
                ...
                if self.okx_open_ok:
                    ...
                    if self.okx_symbol and self.okx_open_price > 0 and self.okx_position_side:
                        has_valid_data = True
                        logger.info(f"✅【价差止损止盈工人】欧易数据有效: 合约={self.okx_symbol}, 价格={self.okx_open_price}, 方向={self.okx_position_side}")
                    else:
                        # 欧易无有效持仓数据，清空欧易缓存
                        self.okx_cache = None
                        self.okx_open_ok = False  # 重置标签，避免重复读取
                        logger.warning("⚠️【价差止损止盈工人】欧易数据无效，跳过")
                ...
                if self.binance_open_ok:
                    ...
                    if self.binance_symbol and self.binance_open_price > 0 and self.binance_position_side:
                        has_valid_data = True
                        logger.info(f"✅【价差止损止盈工人】币安数据有效: 合约={self.binance_symbol}, 价格={self.binance_open_price}, 方向={self.binance_position_side}")
                    else:
                        # 币安无有效持仓数据，清空币安缓存
                        self.binance_cache = None
                        self.binance_open_ok = False  # 重置标签，避免重复读取
                        logger.warning("⚠️【价差止损止盈工人】币安数据无效，跳过")
                
                # 【修改】只要有一个交易所有效就继续，不再强制要求两个都有
                if has_valid_data:
                    return True
                else:
                    logger.warning(f"⚠️【价差止损止盈工人】没有找到任何有效持仓数据 (尝试 {attempt+1}/{max_attempts})")
                ...
                logger.error(f"❌【价差止损止盈工人】读取私人数据失败 (尝试 {attempt+1}/{max_attempts}): {e}")
                if attempt < max_attempts - 1:
                    await asyncio.sleep(1)
        
        logger.error("❌【价差止损止盈工人】所有交易所均无有效持仓数据")
        return False
    ...
    async def _load_okx_tick_sz(self) -> bool:
        ...
        if not self.okx_cache:
            logger.info("📭【价差止损止盈工人】欧易无需处理，跳过精度读取")
            return True
        ...
        for attempt in range(max_attempts):
            try:
                ...
                for contract in contracts:
                    if contract.get("instId") == okx_inst_id:
                        ...
                        if self.okx_tick_sz <= 0:
                            logger.warning(f"⚠️【价差止损止盈工人】欧易tickSz无效: {self.okx_tick_sz}")
                            if attempt < max_attempts - 1:
                                await asyncio.sleep(1)
                                continue
                            return False
                        
                        logger.info(f"✅【价差止损止盈工人】欧易tickSz={self.okx_tick_sz}")
                        return True
                
                logger.warning(f"⚠️【价差止损止盈工人】未找到欧易合约 {okx_inst_id} 的面值数据")
                if attempt < max_attempts - 1:
                    await asyncio.sleep(1)
                ...
                logger.error(f"❌【价差止损止盈工人】读取欧易tickSz失败 (尝试 {attempt+1}/{max_attempts}): {e}")
                if attempt < max_attempts - 1:
                    await asyncio.sleep(1)
    ...
    async def _load_binance_tick_size(self) -> bool:
        ...
        if not self.binance_cache:
            logger.info("📭【价差止损止盈工人】币安无需处理，跳过精度读取")
            return True
        ...
        for attempt in range(max_attempts):
            try:
                ...
                for contract in contracts:
                    if contract.get("symbol") == self.binance_symbol:
                        ...
                        if self.binance_tick_size <= 0:
                            logger.warning(f"⚠️【价差止损止盈工人】币安tickSize无效: {self.binance_tick_size}")
                            if attempt < max_attempts - 1:
                                await asyncio.sleep(1)
                                continue
                            return False
                        
                        logger.info(f"✅【价差止损止盈工人】币安tickSize={self.binance_tick_size}")
                        return True
                
                logger.warning(f"⚠️【价差止损止盈工人】未找到币安合约 {self.binance_symbol} 的精度数据")
                if attempt < max_attempts - 1:
                    await asyncio.sleep(1)
                ...
                logger.error(f"❌【价差止损止盈工人】读取币安tickSize失败 (尝试 {attempt+1}/{max_attempts}): {e}")
                if attempt < max_attempts - 1:
                    await asyncio.sleep(1)
    ...
    def _calculate_prices(self):
        ...
        if self.okx_cache:
            if self.okx_position_side == "long":
                ...
                logger.info(f"📊【价差止损止盈工人】欧易做多: 止损={self.okx_stop_price}, 止盈={self.okx_take_price}")
            elif self.okx_position_side == "short":
                self.okx_stop_price = self.okx_open_price * (1 + sl_abs)
                self.okx_take_price = self.okx_open_price * (1 - tp)
                logger.info(f"📊【价差止损止盈工人】欧易做空: 止损={self.okx_stop_price}, 止盈={self.okx_take_price}")
        ...
        if self.binance_cache:
            if self.binance_position_side == "LONG":
                ...
                logger.info(f"📊【价差止损止盈工人】币安做多: 止损={self.binance_stop_price}, 止盈={self.binance_take_price}")
            elif self.binance_position_side == "SHORT":
                self.binance_stop_price = self.binance_open_price * (1 + sl_abs)
                self.binance_take_price = self.binance_open_price * (1 - tp)
                logger.info(f"📊【价差止损止盈工人】币安做空: 止损={self.binance_stop_price}, 止盈={self.binance_take_price}")
    ...
    def _format_prices(self):
        ...
        if self.okx_cache:
            ...
            logger.info(f"🎯【价差止损止盈工人】欧易精度化后: 止损={self.okx_stop_price}, 止盈={self.okx_take_price}")
        ...
        if self.binance_cache:
            ...
            logger.info(f"🎯【价差止损止盈工人】币安精度化后: 止损={self.binance_stop_price}, 止盈={self.binance_take_price}")
    ...
    def _fill_params(self):
        ...
        if self.okx_cache:
            ...
            logger.info(f"📝【价差止损止盈工人】欧易参数已填充: side={self.okx_cache['params']['side']}, posSide={self.okx_cache['params']['posSide']}")
        ...
        if self.binance_cache:
            ...
            logger.info(f"📝【价差止损止盈工人】币安参数已填充")
    ...
    def _send_to_trader(self):
        ...
        if orders and self.brain.trader:
            ...
            logger.info(f"📤【价差止损止盈工人】已推送 {len(orders)} 个订单给下单工人")
        elif not orders:
            logger.warning("⚠️【价差止损止盈工人】没有需要推送的订单")
    ...
    def _cleanup_work(self):
        ...
        self.spread_strategy_active = False  # 策略标签也清掉，确保每次开仓都需要重新发
        
        logger.info("🧹【价差止损止盈工人】工作缓存已清空，只保留全自动标签")
